...
def populate_distances(valves):
    ...
    for start, _, _ in valves:
        ...
        while unvisited:
            current = min({k:v for k,v in distances.items() if k in unvisited}, key=distances.get)
...
def best_pressure(closed_valves, flows, current, current_time):
    ...
    for valve in closed_valves:
        ...
        total_pressure = (max_time - time) * flows[valve] + best_pressure(closed_valves - {valve}, flows, valve, time)
...
def main():
    ...
    for number_of_my_valves in range(1, len(closed_valves)):
        for my_valves in map(set, itertools.combinations(closed_valves, number_of_my_valves)):
