...
def read_input():
    ...
    with open(filename, "r") as file:
        return np.array(list(map(lambda x: list([ord(char) - 97 for char in x.strip()]), file.readlines())))
...
def expand_node(graph, node):
    ...
    for dx in (-1, 0, 1):
        for dy in(-1, 0, 1):
            ...
            if x >= graph.shape[0] or y >= graph.shape[1]: continue
...
def main():
    ...
    print(min([count_steps(height_map, start, end) for start in zip(*np.where(height_map == ord("a") - 97))]))
